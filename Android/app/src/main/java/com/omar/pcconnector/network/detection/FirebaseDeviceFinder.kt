 ...
 * to his house.
 */
object FirebaseDeviceFinder : DeviceFinder {
    ...
    override suspend fun findDevice(uuid: String): DetectedHost? {
        ...
        try {
            ...
            val ip = deviceInfo.child("ip").getValue(String::class.java)
                ?: return null
            val port = deviceInfo.child("port").getValue(Int::class.java)
                ?: return null

            val api = Retrofit.Builder().baseUrl("http://$ip:$port")
