...
export default class PreferencesManager {
    ...
    private static parseAppPrefsFromJson(json: any): model.AppPreferences {
        ...
        const firebaseServiceData = this.parseFirbaseServiceConfigFromJson(json.global_ip ?? {})
        const detectionServerData = this.parseDetectionServiceConfigFromJson(json.detection_service ?? {})
        const serverInformationData = this.parseServerInformation(json.server_information ?? {})
        const serverConifgData = this.parseServerConfigFromJson(json.server_configuration ?? {})
