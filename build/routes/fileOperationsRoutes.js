...
router.post('/copyResources', authMiddleware, (req, res) => {
    ...
    try {
        copyResources(resourcesPath, pasteDestination, (theFileData, overrallsize, theFiles) => {
            const progress = {
        ...
        if (err instanceof ResourceAlreadyExists) {
            ...
            return res.json(new ErrorResponse(10, "Uncaught error: " + err));
    }
});
router.post('/moveResources', authMiddleware, (req, res) => {
    ...
    try {
        moveResources(resourcesPath, pasteDestination, (theFileData, overrallsize, theFiles) => {
            const progress = {
                currentFile: theFileData.file,
                totalFileBytesCopied: theFileData.totalBytesCopied,
                totalFileSize: theFileData.totalSize,
...
router.post('/deleteResources', authMiddleware, (req, res) => {
    ...
    src = parsePath(src);
    deleteResources(src, permanentlyDelete, (progress) => {
        //res.write(JSON.stringify(progress) + "\n");
    }, () => {
        res.json(new SuccessResponse());
    });
